...
AFND* AFNDMinimiza(AFND * afnd){
	...
	char nombre_estado1_aux[100];
	char nombre_estado2_aux[100];
	...
	for(i=0; i<num_estados_original; i++){
		...
		de_pila_a_estado_2(automata_determinista, stack, &numero_estados_minimo, estados_AFD, i);
	}
	...
	for (z = 0; z < numero_estados_minimo; z++){
		sprintf(nombre_estado1_aux, "q%d", estados_AFD[z].new_id);
			...
			for(z=0; z < numero_estados_minimo; z++){
				for(w=0; w < estados_AFD[z].longitud_estado; w++){
					if(AFNDTransicionIndicesEstadoiSimboloEstadof(automata_determinista, estados_AFD[i].estados[0], j, estados_AFD[z].estados[w]) == 1){
						aniadir_transicion_a_estructura_2(&numero_estados_minimo, estados_AFD, transiciones_AFD, &numero_transiciones_minimo, AFNDSimboloEn(automata_determinista, j), i, z);
	...
	for (i = 0; i < numero_transiciones_minimo; i++){
		sprintf(nombre_estado1_aux, "q%d", transiciones_AFD[i].origen_new_id);
		sprintf(nombre_estado2_aux, "q%d", transiciones_AFD[i].destino_new_id);
